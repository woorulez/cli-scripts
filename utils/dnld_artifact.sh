...
(set -o pipefail; mvn org.apache.maven.plugins:maven-dependency-plugin:2.4:get -Dartifact=$artifact -Ddest=./$filename -DremoteRepositories="$repourl" 2>&1 | tee upgrade.log)
if [ $? -eq 0 ]; then
